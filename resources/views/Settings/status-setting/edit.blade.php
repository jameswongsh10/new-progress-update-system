...
@if(isset($_COOKIE["isLoggedIn"]) && (!strcmp($_COOKIE['user_role'],"admin")))
    ...
    <div class="main">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form autocomplete="off" method="post" action="{{ route('status-setting.update', $currentStatus->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label>Title</label>
                        <input autocomplete="off" type="search" name="title" class="form-control" value="{{$currentStatus->status_title}}"/>
                        <span class="text-danger">@error ('title') {{$message}} @enderror</span>
                    </div>
                    <div class="form-group">
                        <label>Is Active</label>
                        <select name="isActive" class="form-control">
                            <option value="1" @if($currentStatus->is_active == 1) selected @endif>Yes</option>
                            <option value="0" @if($currentStatus->is_active == 0) selected @endif>No</option>
                        </select>
